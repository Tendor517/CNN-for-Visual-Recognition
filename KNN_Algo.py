'''To implment KNN algorithm on existing L1_classifier.py file'''
...
# #l1 distance classifier, my protopye
...
#             distances[self.train_labels[i]]=distance  #storing distances and labels in a dictionary

#         #sorting
#         distances=dict(sorted(distances.items(), key=lambda item: item[1]))
#         k_distances=dict(islice(distances.items(), k))
#         k_labels=list(k_distances.keys())  #list of k labels
#         marjority_label=Counter(k_labels).most_common(1)[0][0]  #majority label
        
#         return marjority_label
...
class L1DistanceClassifier:
    ...
    def predict(self, image, k=3):
        ...
        labels = [label for label, _ in k_nearest_neighbors]
...
if __name__ == '__main__':
    ...
    predict_label = L1_classifier.predict(test_image,k=5)
    print(f"Predicted Label of {test_image_path} = {predict_label}")
